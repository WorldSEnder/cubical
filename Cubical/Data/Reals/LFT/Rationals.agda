...
open import Cubical.Data.Reals.LFT.IntHelpers
  renaming (_≤_ to _≤Int_)

data ℚ : Type₀ where
  _/1+_ : Int → ℕ → ℚ

safePred : (n : ℕ) → ¬ (n ≡ 0) → ℕ
safePred 0 ¬0 = ⊥-elim (¬0 refl)
safePred (suc n) ¬0 = n

_over-signed_by_ : Int → (m : Signed) → ¬ (m ≡ neut) → ℚ
n over-signed posSuc x by ¬m≡0 = n /1+ x
n over-signed neut by ¬m≡0 = ⊥-elim (¬m≡0 refl)
n over-signed negSuc x by ¬m≡0 = n /1+ x

_over_by_ : Int → (n : Int) → ¬ (n ≡ Int.pos 0) → ℚ
n over m by ¬m≡0 = n over-signed (Int→Signed m) by ¬sm≡neut where
  open Iso
